package me.xemor.stoprenaming.commands;
...
public class StopRenamingCommand implements CommandExecutor, TabExecutor {

    public String[] subCommands = new String[]{"reload"};
    private Rename rename;

    public StopRenamingCommand(Rename rename) {
        this.rename = rename;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            if ("reload".equals(args[0])) {
                if (sender.hasPermission("stoprenaming.reload")) {
                    rename.reloadConfig();
